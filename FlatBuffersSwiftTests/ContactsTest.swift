...
class ContactsTest: XCTestCase {
    ...
    func testSmall(){
        let list =  ContactList(lastModified: 123, entries: [
            Contact(name: "Maxim", birthday: nil, gender: nil, tags: ["a", "b"])
        ])
        ...
        XCTAssertEqual(readContactList.lastModified, 123)
        XCTAssertEqual(readContactList.entries.count, 1)
    ...
    func createContactList() -> ContactList {
        ...
        let item1 = Contact(name: "Maxim",
                            ...
                            tags: ["h1", "h2", "h3"],
                            addressEntries: [
                                AddressEntry(order: 0, address: Address.withEmailAddress(EmailAddress(mailto: "bla@bla.io"))),
                                AddressEntry(order: 1, address: Address.withPostalAddress(PostalAddress(country: "DE", city: "Berlin", postalCode: 13000, streetAndNumber: "Balstr, 23"))),
                                AddressEntry(order: 2, address: Address.withWebAddress(WebAddress(url: "http://slkf.com"))),
                                AddressEntry(order: 3, address: Address.withTelephoneNumber(TelephoneNumber(number: "+4923452425")))
                            ],
